...
for(qq in my_contours){
  ...
  lines(ellipse(x = cov(dat_kenya), centre = colMeans(dat_kenya), 
...
dat_feast_hrp2$type=2
  ind_0parasite = which(!is.na(dat_feast_hrp2$) & dat_feast$parasitaemia==0)
dat_feast$type[ind_0parasite] = 3
points(log10(dat_feast$platelets), log10(dat_feast$hrp2),
       pch=c(15,17)[(dat_feast$type-1)], 
       col= adjustcolor(mycols[dat_feast$type], .6), cex=1.2)

ind_spline = !is.na(dat_feast$platelets) & dat_feast$platelets>10
f=smooth.spline(y = log10(dat_feast$hrp2[ind_spline]),
                x = log10(dat_feast$platelets[ind_spline]))
...
dat_feast$type=2
ind_0parasite = which(!is.na(dat_feast$parasitaemia) & dat_feast$parasitaemia==0)
dat_feast$type[ind_0parasite] = 3
points(log10(dat_feast$platelets), log10(dat_feast$hrp2),
       pch=c(15,17)[(dat_feast$type-1)], 
       col= adjustcolor(mycols[dat_feast$type], .6), cex=1.2)
...
panels = c('','')
plotting_dat = rbind(dat_Training, dat_kenya[, 1:2])
study = c(rep(1, nrow(dat_Training)), rep(2, nrow(dat_kenya)))
ind_rand = sample(1:nrow(plotting_dat), nrow(plotting_dat), replace = F)
...
points(dat_kenya[dat_kenya$HbAS==1, 1], 
       dat_kenya[dat_kenya$HbAS==1, 2],
       col='black',pch=18)
axis(side = 1, at = axis_points[[1]], 
     labels = 10^axis_points[[1]])
axis(side = 2, at = axis_points[[2]], 
...
dat_feast$SM = as.numeric(dat_feast$hrp2>=1000)
mod = mgcv::gam(SM ~ s(log(platelets)), family='binomial', data=dat_feast)
summary(mod)
...
plot(xs, inv.logit(ys$fit), ylim = c(0,1),main = 'Severely ill African children (FEAST)',
     type='l', ylab='Probability PfHRP2>1000 ng/mL', panel.first=grid(),
...
dat_feast$parasites = as.numeric(dat_feast$parasitaemia>=10000)
mod = mgcv::gam(parasites ~ s(log(platelets)), 
                family='binomial', data=dat_feast)
summary(mod)
xs = seq(1,3, length.out = 1000)
ys=predict(mod, newdata = data.frame(platelets=10^xs), se.fit = T)
uppers = inv.logit(ys$fit + 1.96*ys$se.fit)
...
dat_kenya$Ref_Model_Likelihood = dmvt(x = dat_kenya[, 1:2], 
                                      delta = Model_mu, 
                                      sigma = Model_sigma, 
                                      df = 7)
hist(dat_kenya$Ref_Model_Likelihood, xlab='Likelihood under the reference model',main='')
...
Testing_data = dat_kenya[, 1:2] # age-corrected wbc

stan_data = list(D=2,
                 K_notSM=6,
                 N_Training_Africa = nrow(feast_dat_training),
                 N_Training_Asia = nrow(imputed_data_Viet[[1]])+nrow(core_SM),
                 N_Testing = nrow(Testing_data),
                 y_Training_Africa = feast_dat_training,
                 y_Training_Asia = rbind(imputed_data_Viet[[1]][,biomarker_vars],core_SM),
...
ws_dat = data.frame(IID = dat_kenya$IID,
                    ws = ps_post)
save(ws_dat, file = 'patient_SM_weights2.RData')
```
...
```{r}
upper_p = round(quantile(dat_kenya$P_SMs, probs=.8),1)
print(upper_p)
lower_p = round(quantile(dat_kenya$P_SMs, probs=.2),1)
print(lower_p)
