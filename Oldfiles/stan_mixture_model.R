...
mixture_model = '
  ...
  model {
      real ps[2];
      // prior
      mu_SM ~ normal(mu_prior_SM,.05);
      mu_not_SM ~ normal(mu_prior_not_SM, .1);
      ...
      // likelihood
      y_SM ~ multi_student_t(nu_1, mu_SM, Sigma_SM);
      for (n in 1:N_latent){

        ps[1] = log(theta) + multi_student_t_lpdf(y_latent[n] | nu_1, mu_SM, Sigma_SM);
        ps[2] = log1m(theta) + multi_student_t_lpdf(y_latent[n] | nu_2, mu_not_SM, Sigma_not_SM);
  ...
  generated quantities {
    ...
    for(n in 1:N_latent){
      ...
      // SM
      prob_labels_raw[1] = theta*exp(multi_student_t_lpdf(y_latent[n] | nu_1, mu_SM, Sigma_SM));
      prob_labels_raw[2] = exp(1-theta)*exp(multi_student_t_lpdf(y_latent[n] | nu_2, mu_not_SM, Sigma_not_SM));
      // normalise to 1
...
mix_model = stan_model(model_code = mixture_model)

# dat1 = rbind(mvtnorm::rmvnorm(n = 50, mean = c(0,0)),
#             mvtnorm::rmvnorm(n = 50, mean = c(1,1)))
# dat2 = mvtnorm::rmvnorm(n = 500, mean = c(0,0))
#
#
# out_mod=sampling(object = mix_model,
#                  data = list(D=2,
#                              K=2,
#                              N_latent = nrow(dat1),
#                              N_obs = nrow(dat2),
...
#                              mu_1 = c(0,0)))
# xx = extract(out_mod)
# mus = apply(xx$mu, MARGIN = 2:3, FUN = mean)
# mus
